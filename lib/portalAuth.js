...
async function requirePortalSession(req) {
  ...
  if (viewAsRole) {
    ...
    // If a specific person is selected, treat this as true impersonation.
    impersonating = !!effectiveUserId;

    // Role-only view-as (no person selected) stays read-only to avoid writes
    // that can't be attributed to a single user.
    const m = String(req?.method || 'GET').toUpperCase();
    if (!['GET', 'OPTIONS', 'HEAD'].includes(m) && !impersonating) {
      return { ok: false, error: 'view_as_requires_user', status: 403 };
    }
  }
