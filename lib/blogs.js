const { getJson, setJson, delKeys } = require('./storage');
...
async function deleteAllPosts() {
  const index = await getIndex();
  const keys = [];
  for (const slug of index) keys.push(POST_KEY_PREFIX + String(slug));
  keys.push(INDEX_KEY);
  keys.push(STATE_KEY);
  await delKeys(keys);
  return { ok: true, deleted: Math.max(0, index.length) };
}
