...
const { createPreviewSession, supabaseAdmin, verifyPreviewToken } = require('../../lib/portalAuth');
...
module.exports = async (req, res) => {
  ...
  const preview = verifyPreviewToken(refreshToken);
  if (preview.ok) {
    const session = await createPreviewSession(sbAdmin, { userId: preview.payload.userId });
    if (!session.ok) {
      return sendJson(res, session.status || 403, { ok: false, error: session.error || 'preview_auth_failed' });
    }
