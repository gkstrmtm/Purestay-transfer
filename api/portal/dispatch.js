...
const { ensureDispatchWorkOrder, tableExists } = require('../../lib/portalFoundation');
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    const hasWorkOrders = await tableExists(s.sbAdmin, 'portal_dispatch_work_orders');
    let workOrdersByEvent = new Map();
    if (hasWorkOrders && tasks.length) {
      const taskIds = tasks.map((t) => Number(t?.id || 0)).filter((id) => Number.isFinite(id) && id > 0);
      const { data: workOrders } = await s.sbAdmin
        .from('portal_dispatch_work_orders')
        .select('event_id, owner_user_id, status, dispatch_type, priority, vendor_dependency_count, updated_at, notes')
        .in('event_id', taskIds);
      workOrdersByEvent = groupByEventId(workOrders);
    }

    return sendJson(res, 200, {
      ok: true,
      tasks: tasks.map((task) => Object.assign({}, task, {
        workOrder: workOrdersByEvent.get(Number(task?.id || 0)) || null,
      })),
    });
  }
  ...
  if (req.method === 'POST') {
    ...
    await ensureDispatchWorkOrder(s.sbAdmin, {
      event: inserted,
      ownerUserId: assignedUserId || s.actorUserId,
      status: inserted?.status || 'open',
      dispatchType: 'lead_followup',
      priority,
      notes: inserted?.notes || '',
      meta: { source: 'api/portal/dispatch' },
    }).catch(() => null);
  ...
  if (req.method === 'PATCH') {
    ...
    if (String(body.action || '') === 'escalate') {
      ...
      await ensureDispatchWorkOrder(s.sbAdmin, {
        event: updated,
        ownerUserId: updated?.assigned_user_id || s.actorUserId,
        status: updated?.status || 'open',
        dispatchType: meta.dispatchType || 'lead_followup',
        priority: nextMeta.priority,
        notes: updated?.notes || '',
        meta: { source: 'api/portal/dispatch', escalated: true },
      }).catch(() => null);
