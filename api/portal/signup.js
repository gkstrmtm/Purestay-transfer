...
const { ALLOWED_PORTAL_ROLES, normalizePortalRole, cleanStr, syncPortalPerson } = require('../../lib/portalFoundation');

function selfSignupEnabled() {
  return String(process.env.PORTAL_ALLOW_SELF_SIGNUP || '').trim() === '1';
}
...
module.exports = async (req, res) => {
  ...
  if (!selfSignupEnabled()) {
    return sendJson(res, 403, {
      ok: false,
      error: 'self_signup_disabled',
      hint: 'Use manager provisioning or an invite-based activation flow instead.',
    });
  }
  ...
  const personSync = await syncPortalPerson(sb, {
    userId,
    role,
