...
const { ensureDispatchWorkOrder } = require('../../lib/portalFoundation');
...
async function insertDispatch(sbAdmin, { actorId, assignedRole, dueDate, title, notes, followupKey, eventId }) {
  ...
  return { ok: true, taskId: row?.id || null, task: row || null };
}
...
module.exports = async (req, res) => {
  ...
  async function ensureTask(key, assignedRole, title, notes) {
    ...
    if (r.ok) {
      created.push({ key, taskId: r.taskId });
      if (r.task) {
        await ensureDispatchWorkOrder(s.sbAdmin, {
          event: r.task,
          ownerUserId: r.task.assigned_user_id || r.task.created_by,
          status: 'open',
          dispatchType: key,
          meta: { followupKey, source: 'api/portal/event_followups', parentEventId: eventId },
        }).catch(() => {});
      }
    }
  }
