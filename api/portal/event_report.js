...
const { listAccounts, findAccountForEvent } = require('../../lib/portalAccounts');
const { upsertEventClosureRecord, upsertEventSurveyDistribution } = require('../../lib/portalFoundation');
...
async function deriveAccountOwnerEmails(sbAdmin, ev) {
  const listed = await listAccounts(sbAdmin);
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    const surveyResult = await upsertEventSurveyDistribution(s.sbAdmin, {
      eventId,
    ...
    const closureResult = await upsertEventClosureRecord(s.sbAdmin, {
      eventId,
      closureStatus: String(nextChecklist.internalLogReceived || '') === 'yes' ? 'in_review' : 'not_started',
      formsComplete: String(nextChecklist.feedbackReceived || '') === 'yes',
      reportComplete: true,
      notes: cleanStr(meta?.reportDraft?.plainText || plainText || '', 4000),
      meta: {
        reportSentAt: sentAt,
        reportRecipients: recipients,
      },
    ...
    const { data: updatedRows } = await s.sbAdmin
      .from('portal_events')
      .update({ meta: nextMeta, report_status: 'submitted' })
      .eq('id', eventId)
      .select('*')
      .limit(1);
    ...
    return sendJson(res, 200, {
      ...
      providerId: result.id || null,
      event: updatedEvent,
      closure: closureResult?.closureRecord || null,
      surveyDistribution: surveyResult?.surveyDistribution || null,
    });
  }
